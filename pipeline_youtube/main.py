"""Console entrypoint for pipeline-youtube.
...
``stages`` / ``transcript`` / ``synthesis`` packages and the extracted steps
(``cli_validation`` / ``runtime`` / ``input_resolver`` / ``execution_plan`` /
``pipeline_runner`` / ``synthesis_runner`` / ``reporting``).
