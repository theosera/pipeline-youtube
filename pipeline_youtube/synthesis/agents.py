...
def call_alpha_batched(
    ...
    *,
    ...
    # Parallel α is all-or-nothing: dropping a failed batch would silently
    # remove those videos' topics from β/Leader while reporting success.
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in as_completed(future_to_index):
            ...
            try:
                ...
                continue
            if not topics:
                failures.append((idx, SynthesisParseError("α batch returned no topics")))
                _LOG.warning("α batch %d/%d returned no topics", idx + 1, len(batches))
                continue
    ...
    if failures:
        ...
        raise SynthesisParseError(
            f"{len(failures)}/{len(batches)} α batches failed; first error: {first_exc!r}"
        )
