...
def call_alpha_batched(
    ...
    *,
    ...
    # Keep the executor isolation so one exception does not cancel sibling
    # futures, but require every batch to succeed before merging. Returning a
    # partial merge would silently omit videos from the final synthesis.
    ...
    if failures:
        ...
        failed_batches = ", ".join(str(idx + 1) for idx, _exc in sorted(failures))
        first_exc = sorted(failures, key=lambda item: item[0])[0][1]
        raise SynthesisParseError(
            f"{len(failures)}/{len(batches)} α batches failed "
            f"(failed batches: {failed_batches}); first error: {first_exc!r}"
        )
