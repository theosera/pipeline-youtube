...
def call_alpha_batched(
    ...
    *,
    ...
    # Run batches independently for concurrency, but fail closed if any
    # batch fails. A partial α result silently drops videos/topics from
    # the final synthesis while the stage still writes "successful" notes.
    ...
    if failures:
        ...
        raise SynthesisParseError(
            ...
            f"first failure batch {first_idx + 1}: {first_exc!r}"
        )
