...
def call_alpha_batched(
    ...
    *,
    ...
    # Collect every future before deciding whether to fail. Returning
    # successful batches after any failed batch would silently drop that
    # batch's videos from the final synthesis.
    ...
    if failures:
        ...
        failed_batches = ", ".join(str(idx + 1) for idx, _ in sorted(failures))
        raise SynthesisParseError(
            f"{len(failures)}/{len(batches)} α batches failed "
            f"(batch(es): {failed_batches}); first error: {first_exc!r}"
        )
