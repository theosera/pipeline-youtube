...
def fetch_video_extra_metadata(video_id: str, *, timeout: int = 30) -> VideoExtraMetadata:
    ...
    Returns an empty result on any failure so the caller can fall back
    gracefully — this module has no persistent cache (this repo doesn't
    have a caching layer), so a failed fetch is simply retried next run.
    """
    opts = {
        "quiet": True,
        "no_warnings": True,
    ...
    return VideoExtraMetadata(description=description, chapters=chapters)
