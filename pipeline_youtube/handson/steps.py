...
) -> tuple[StepBody, list[ClaudeResponse]]:
...
def generate_moc_summary(
    ...
    *,
    model: str,
) -> tuple[HandsonMocOutput, list[ClaudeResponse]]:
    ...
    try:
        response = invoke_claude(
            prompt=prompt,
            append_system_prompt=MOC_SYSTEM_PROMPT,
            model=model,
    ...
    except (ClaudeCliError, HandsonParseError):
