...
#
# The embed target must allow a literal ``]`` that is not the ``]]``
...
# first ``]`` drops every mapping for those playlists (empty allow-list →
# Stage 04 emits "画像無し"). Same rule as ``services.confusables``.
