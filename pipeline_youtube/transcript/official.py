...
def fetch_official(
    ...
    ``stages/scripts.py``) instead of a module-global singleton, so a fresh
    ``YouTubeTranscriptApi`` (its own ``requests.Session``) is created per call
    rather than shared process-wide (the parallelization blocker this DI
    removes; migration §3-6/§6-3).
    """
    if not video_id:
