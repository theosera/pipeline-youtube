...
01b が訂正を行うと、01 ノートの隣に **`{動画名} — corrections.md`** を出力する。変更のあった
箇所ごとに **before→after / 訂正理由 / web 検索で参照した URL** を記録するので、「なぜその訂正に
なったか」を後から検証できる。変更が無ければレポートは作られない（`--dry-run` でも作らない）。
