...
2 系統ある: (a) 不可視除去・混在スクリプト**検出**と alert 記録は fetch 境界
`playlist.fetch_metadata` (→ `analyze_filename_text`) が**唯一の記録点**で行い、下流の
`sanitize_title_for_filename` / `build_frontmatter` (title/filename 経路) は不可視文字を
防御的に再除去するのみ (混在スクリプト検出も再 alert もしない)。(b) 混在スクリプト **fold**
