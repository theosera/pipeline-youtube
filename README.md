...
> ⚠️ **個人利用を前提とした実験的リポジトリです**
> 本リポジトリは、作者自身が YouTube 動画教材を Obsidian に整理するために作った私的な実験用パイプラインです。
> - 一般向けの製品ではなく、動作保証・サポート・後方互換は提供しません (設計変更は予告なく発生します)
> - 動画由来の生成物 (文字起こし / 要約 / 画面キャプチャ / 章別 md) は **利用者自身のローカル環境での個人学習目的のみ** を想定しています
> - **公開・共有・商用利用を行う前に [権利・利用上の注意](#権利利用上の注意) を必ず読んでください**

YouTube プレイリスト → Obsidian Vault 自学自習用学習レポート生成パイプライン。

NotebookLM に動画 URL を 1 本ずつ手動で貼り付けて要約を Obsidian に転記する運用を自動化する。プレイリスト URL を 1 本投げると、プレイリスト内の全動画を以下の工程に通して、学習しやすいレポートと分野横断のハンズオンを **自分の端末内の Obsidian Vault** に保存する。ネットワーク外に成果物をアップロードする機能は実装していない。
...
**プレイリスト単位 (05)**
- **05_Synthesis**: プレイリストの動画数 ≥ 3 本の時、Agent Teams (α→β→Leader) で 04 全体を横断統合し、章別ハンズオン md 群 + 00_MOC.md + 重複度スコア JSON を出力する。カバレッジ検証は Python 集合演算で決定論的に実施し、β の漏れトピックは Reflexion リトライで自己修復する。動画本数に応じて `standard` / `parallel` / `full` プロファイルを自動選択 (詳細は [`docs/agent-teams-profiles.md`](docs/agent-teams-profiles.md))
...
本リポジトリは、YouTube 等の動画教材を**個人学習用に整理するための実験的パイプライン**です。

本リポジトリには、**第三者が権利を有する動画の画面キャプチャ、サムネイル、字幕、要約済み教材、生成済み Markdown 等は一切含めていません**。git 管理対象はパイプラインのソースコード・テスト・設定テンプレート・ドキュメント・プロンプトのみです。Vault への出力・画面キャプチャ・動画の一時ダウンロードは、すべて **リポジトリ外のパス** (`config.json` の `vault_root` および OS の一時ディレクトリ) に書き込まれるため、そもそも git で追跡されません。個人設定ファイル・ログ・テンポラリは `.gitignore` で追加的に除外しています。

動画由来の生成物は、**利用者自身のローカル環境における個人学習目的** でのみ扱うことを想定しています。

生成物を**公開・共有・商用利用する場合は、元動画の権利者の許諾、各プラットフォームの利用規約、著作権法上の引用要件等を必ず確認**してください。本パイプラインが出力する Markdown/画像はそのままでは配布適格性を保証するものではありません (引用の主従関係、出所明示、改変可否などは利用者側で判断すべき事項です)。

### このリポジトリに含まれるもの / 含まれないもの

| 種別 | git 管理対象 | 備考 |
|---|---|---|
| パイプライン本体のコード | ✅ | `pipeline_youtube/`, `tests/`, `docs/`, `docker/` |
| プロンプト定数 | ✅ | `pipeline_youtube/synthesis/agents.py` の ALPHA/BETA/LEADER/REVIEWER |
| 設定テンプレート | ✅ | `config.example.json` のみ。個人設定は `config.json` (gitignore) |
| YouTube 動画のダウンロード物 | ❌ | yt-dlp が一時ディレクトリにのみ保存、処理後削除 |
| 画面キャプチャ (WebP/GIF) | ❌ | ユーザーの Vault 内 `_assets/` にのみ書き込み |
| 字幕・文字起こし (01_Scripts.md) | ❌ | ユーザーの Vault 内にのみ書き込み |
| 要約・学習教材 (02/04/05 の md) | ❌ | ユーザーの Vault 内にのみ書き込み |
| プレイリスト/動画メタデータ | ❌ | 処理時にのみ取得、永続化しない |
| Claude OAuth セッション | ❌ | `~/.claude/` に格納。リポジトリ外 |

### データフローの前提

- ネットワーク送信先は **YouTube (字幕/動画取得)** と **Anthropic API (claude CLI 経由、OAuth セッション)** のみ。生成物を外部に publish する機能はありません
- すべての出力は **ローカルの Obsidian Vault** にのみ書き込まれます
- パイプラインは `claude login` 済みのユーザー個人アカウントのクレジットを消費します (Claude Pro/Max 定額前提)。共有アカウント・API キー運用は想定していません

### 免責

本リポジトリのコードは **AS-IS** で提供されます。生成物の精度・完全性・著作権適法性は保証されず、利用によって生じた損害 (データ損失、規約違反に伴うアカウント停止、権利者とのトラブル等) について作者は一切の責任を負いません。
...
#     {"router","stage_02","stage_04","alpha","beta","leader","reviewer"} のキーを任意に設定。
...
| `--synthesis-profile {auto,standard,parallel,full,parallel+full}` | Stage 05 の Agent Teams 構成。`auto` は動画本数で自動選択。`full` は Reviewer を追加、`parallel` は α をバッチ並列実行 ([`docs/agent-teams-profiles.md`](docs/agent-teams-profiles.md)) |
...
| **reviewer** (ε, `full` プロファイル時のみ) | 規約遵守の最終校正 | Leader 出力 + α + β + coverage | `ReviewerFeedback` (`needs_revision`, `fixes[]`)。必要なら Leader を 1 回再生成 |

### プロファイル (動画本数・品質要件に応じた構成切替)

| プロファイル | 構成 | 自動選択の目安 |
|---|---|---|
| `standard` | α → β → Leader | n_videos ≤ 15 (既存動作) |
| `parallel` | α(バッチ並列) → merge → β → Leader | 16 ≤ n_videos ≤ 30 |
| `full` | α → β → Leader → Reviewer | 手動指定時のみ (公開用途など) |
| `parallel+full` | α(並列) + Reviewer | n_videos > 30 (自動) |
...
- **隠蔽 (homoglyph / 不可視・bidi) 防御** — `services/confusables`
  外部由来テキスト (YouTube タイトル、および LLM 生成本文) の隠蔽系に多層で対処する。(1) fetch 境界 (`playlist.fetch_metadata` → `analyze_filename_text`) が**唯一の alert 記録点**で、取得タイトルの不可視/双方向/ゼロ幅文字および Latin×Cyrillic/Greek の混在スクリプト (古典的 homoglyph 偽装) を**検出**して alert に記録する (**検出のみ** — emit する `VideoMeta` は生タイトルのままで、ここでは除去しない)。不可視文字の**実際の除去**は下流のファイル名・frontmatter チョークポイント (`sanitize_title_for_filename` / `build_frontmatter` → `strip_invisibles`) が担い、そこでは混在スクリプト検出も再 alert も行わない (read/dedup 走査での重複 alert を避けるため)。(2) LLM 出力 (OpenAI/Claude が稀に吐く Cyrillic/Greek lookalike) を vault へ書き出す**直前**に、混在スクリプトの letter-run 語について、マップ済みの Cyrillic/Greek lookalike を**すべて** Latin へ **fold** する (`fold_mixed_script_confusables`) — Stage 02 summary と Stage 05 synthesis (chapter / MOC) の LLM 出力経路**のみ** (Stage 01 の字幕本文は fold 対象外)、`validate_chapter_body` の HTML 無害化の**前段**に配置。fold は決定論・冪等・混在 letter-run 語限定。「無変更」の保証は**純スクリプト run** に対して成立し (日本語・純キリル/ギリシャ語、および URL/markdown 内の正当な非ラテンは letter-run 分割により純スクリプト語として無変更)、混在 run 内はその letter-run 内のマップ済み lookalike を全置換する。既存ノートの read-only 外部監査は `homoglyph-audit` skill、リネーム是正は `scripts/remediate_filenames.py --apply`。
...
# 605 passed, 1 skipped (2026-04-24 時点)
```
...
- [`docs/agent-teams-profiles.md`](docs/agent-teams-profiles.md) — Stage 05 のプロファイル運用指針 (standard / parallel / full / parallel+full)
- [`docs/docker.md`](docs/docker.md) — Stage 03 の Docker 隔離バックエンド
