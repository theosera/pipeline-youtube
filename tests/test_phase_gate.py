...
class TestFilterToReviewed:
    ...
    def test_older_reviewed_summary_wins_over_newer_unreviewed(self, tmp_path: Path):
        ...
        config.set_vault_root(tmp_path)
        older = tmp_path / LEARNING_BASE / UNIT_DIRS["summary"] / "2026-04-18-0800 testlist"
        newer = tmp_path / LEARNING_BASE / UNIT_DIRS["summary"] / "2026-04-18-1000 testlist"
        ...
        kept = _filter_to_reviewed(
            [(1, _vid(_VID_A))], "testlist", resume_time, vault_root=config.get_vault_root()
        )
        ...
        assert (
            _find_summary_md(_VID_A, "testlist", resume_time, vault_root=config.get_vault_root())
            == newer / "a.md"
        )
...
class TestCollectExistingLearningBodies:
    ...
    def test_same_day_superset_title_does_not_hide_earlier_day(self, tmp_path: Path, capsys):
        ...
        base = tmp_path / LEARNING_BASE / UNIT_DIRS["learning"]
        _write_learning(base / "2026-04-17-2100 testlist" / "note.md", _VID_1)
        _write_learning(base / "2026-04-18-0800 testlist Advanced" / "other.md", "otherxxxxxA")
    ...
    def test_same_day_superset_title_does_not_steal_overlapping_video_ids(self, tmp_path: Path):
    ...
    def test_empty_exact_same_day_folder_falls_through_to_earlier_day(self, tmp_path: Path, capsys):
...
class TestResumeReviewedProcessing:
    ...
    def test_checkpoint_skip_loads_04_from_newest_same_day_folder(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        """Evening checkpoint must not feed Stage 05 the morning body.

        Two same-day exact-title 04 folders exist; iterdir yields the older
        one first. ``_load_existing_04_body`` shares ``_find_learning_folder``
        with the skip decision, so a first-match fallback would synthesize
        the stale morning note after an afternoon --force-video rewrite.
        """
        config.set_vault_root(tmp_path)
        morning = tmp_path / LEARNING_BASE / UNIT_DIRS["learning"] / "2026-04-18-0800 testlist"
