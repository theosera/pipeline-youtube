...
def _response(text: str) -> ClaudeResponse:
    return ClaudeResponse(text=text, model="sonnet", input_tokens=10, output_tokens=10)
...
class TestPlanSteps:
    def test_complete_assignment_needs_one_call(self, monkeypatch):
        ...
        plan, responses = plan_steps(
            _video(), _segments(), _insights(), duration=9000, model="sonnet"
        )
        assert calls["n"] == 1
    ...
    def test_missing_id_triggers_retry_listing_it(self, monkeypatch):
        ...
        monkeypatch.setattr(planner_mod, "invoke_claude", fake_invoke)
        plan, _ = plan_steps(_video(), _segments(), _insights(), duration=9000, model="sonnet")
    ...
    def test_no_lecture_segments_still_produces_a_step(self, monkeypatch):
        ...
        plan, _ = plan_steps(_video(), qa_only, [], duration=9000, model="sonnet")
