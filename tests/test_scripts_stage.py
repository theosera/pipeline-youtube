...
class TestRunStageScripts:
    ...
    def _stub_extra_metadata(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            scripts_stage, "fetch_video_extra_metadata", lambda video_id: _EMPTY_EXTRA
        )
    ...
    def test_correction_feeds_corrected_text_downstream(self, vault, monkeypatch):
        ...
        def _fake_correct(chunks, *, model, known_terms=None, description=None):
    ...
    def test_description_and_chapters_attached_to_result(self, vault, monkeypatch):
        ...
        monkeypatch.setattr(scripts_stage, "fetch_video_extra_metadata", lambda video_id: extra)
        monkeypatch.setattr(
            scripts_stage,
            "fetch_with_fallback",
            lambda video_id, languages, fetchers: _fake_fetch_success()(video_id, languages),
    ...
    def test_description_reaches_correction_as_known_context(self, vault, monkeypatch):
        ...
        def _spy_correct(chunks, *, model, known_terms=None, description=None):
    ...
    def test_local_media_skips_extra_metadata_fetch(self, vault, monkeypatch, tmp_path):
        ...
        result = scripts_stage.run_stage_scripts(
            video, scripts_path, window_seconds=30.0, media_path=media_path
        )
    ...
    def test_coding_playlist_reuses_fetched_description_no_double_call(self, vault, monkeypatch):
        ...
        monkeypatch.setattr(scripts_stage, "fetch_snippets_for_urls", lambda urls: [])

        scripts_stage.run_stage_scripts(
            video, scripts_path, window_seconds=30.0, include_code_blocks=True
        )
