...
class TestErrorHandling:
    ...
    def test_empty_alpha_topics_returns_error(self, vault, monkeypatch):
        _mock_all_agents(monkeypatch, responses=[json.dumps({"topics": []}), BETA_OUT, LEADER_OUT])
        videos = [_video(i) for i in range(1, 4)]
        result = run_stage_synthesis(
            videos,
            ["b1", "b2", "b3"],
        ...
        assert result.error is not None
        assert "alpha_parse_failed" in result.error
        assert "no topics" in result.error
        assert result.moc_path is None
        assert result.chapter_paths == []
