...
class TestCallAlphaBatched:
    ...
    def test_single_batch_failure_raises(self, monkeypatch):
        # Batch 1 returns valid α JSON; batch 2 returns garbage that
        # fails to parse. Returning only batch 1 would silently drop the
        # videos from batch 2, so the α stage must fail as a whole.
        ...
        videos = [_video(f"vid{i}", f"t{i}") for i in range(1, 5)]
        bodies = [f"body{i}" for i in range(1, 5)]
        with pytest.raises(SynthesisParseError, match="1/2 α batches failed"):
            call_alpha_batched(
                videos,
                bodies,
                batch_size=2,
    ...
    def test_all_batches_failing_raises(self, monkeypatch):
        ...
        videos = [_video(f"vid{i}", f"t{i}") for i in range(1, 5)]
        bodies = [f"body{i}" for i in range(1, 5)]
        with pytest.raises(SynthesisParseError, match="2/2 α batches failed"):
