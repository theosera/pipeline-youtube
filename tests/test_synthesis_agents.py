...
class TestCallAlphaBatched:
    ...
    def test_single_batch_failure_raises(self, monkeypatch):
        ...
        # fails to parse. Returning only batch 1 would silently omit
        # batch 2's videos from the final synthesis.
        ...
        videos = [_video(f"vid{i}", f"t{i}") for i in range(1, 5)]
        bodies = [f"body{i}" for i in range(1, 5)]
        with pytest.raises(SynthesisParseError, match="1/2 .* batches failed"):
            call_alpha_batched(
                videos,
                bodies,
                batch_size=2,
    ...
    def test_all_batches_failing_raises(self, monkeypatch):
        ...
        videos = [_video(f"vid{i}", f"t{i}") for i in range(1, 5)]
        bodies = [f"body{i}" for i in range(1, 5)]
        with pytest.raises(SynthesisParseError, match="2/2 .* batches failed"):
