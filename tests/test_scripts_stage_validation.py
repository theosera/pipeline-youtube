"""Stage 01 must validate the body before it reaches the vault (S03 / PR-06).
...
def _stub_extra_metadata(monkeypatch):
    monkeypatch.setattr(scripts_stage, "fetch_video_extra_metadata", lambda video_id: _EMPTY_EXTRA)
...
def _run(vault: Path, monkeypatch, texts: list[str], **kw) -> Path:
    ...
    scripts_stage.run_stage_scripts(video, paths["scripts"], window_seconds=30.0, **kw)
...
class TestFetchedCodeIsValidatedToo:
    ...
    def _run_with_snippet(self, vault, monkeypatch, content: str) -> str:
        monkeypatch.setattr(
            ...
            lambda video_id: VideoExtraMetadata(
...
class TestInputLengthCeiling:
    def test_oversized_body_is_refused_and_nothing_is_written(self, vault, monkeypatch):
        ...
        with pytest.raises(scripts_stage.ScriptBodyTooLargeError):
            scripts_stage.run_stage_scripts(video, paths["scripts"], window_seconds=30.0)
