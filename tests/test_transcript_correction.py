...
def _response(text: str, *, cost: float | None = None) -> ClaudeResponse:
    return ClaudeResponse(text=text, model="opus", total_cost_usd=cost)
...
class TestCorrectChunks:
    ...
    def test_no_known_terms_leaves_base_prompt(self) -> None:
        ...
        correct_chunks(self._chunks(), model="opus", invoke=invoke)
        assert "確定済み固有名詞辞書" not in invoke.calls[0]["append_system_prompt"]  # type: ignore[attr-defined]
    ...
    def test_no_description_leaves_base_prompt(self) -> None:
        invoke = _stub_invoke('[{"idx": 0, "text": "G"}, {"idx": 1, "text": "x"}]')
        correct_chunks(self._chunks(), model="opus", invoke=invoke)
        assert "動画概要欄" not in invoke.calls[0]["append_system_prompt"]  # type: ignore[attr-defined]
    ...
    def test_description_context_does_not_leak_into_prompt(self) -> None:
        ...
        payload — it must land in append_system_prompt only, never in the
        per-batch user prompt (which carries the transcript chunks)."""
        invoke = _stub_invoke('[{"idx": 0, "text": "G"}, {"idx": 1, "text": "x"}]')
        correct_chunks(
            self._chunks(),
            model="opus",
