"""Glossary substitution must happen before validation (S17 / PR-43).
...
class TestOneLinerStillNormalizes:
    ...
    It used to be normalized by its own ``normalize_text`` call after
    validation; that call is gone, so this pins that moving the substitution
    ahead of ``_extract_one_liner`` did not quietly drop it.
    """
...
class TestSubstitutionCannotSteerTheFormat:
    ...
    This repo has no repair loop (the SDK does), so a structural failure
    propagates as ``SummaryOutputError`` and nothing is written. Same
    ...
    def test_a_canonical_cannot_fake_the_required_sections(self, vault, monkeypatch):
        ...
        satisfied it, were then stripped, and nothing raised — a summary with
